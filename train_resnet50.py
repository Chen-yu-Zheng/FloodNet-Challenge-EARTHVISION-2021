...
class SANExperiment():
    def __init__(self, train_set, output_dir, batch_size, num_epochs, lr,early_stopping=False):
        ...
        # init model
        self.image_model = ResNet34(output_features=1024).to(self.device)
        ...
        for epoch in range(start_epoch, self.num_epochs):
            ...
            for i, q, s, a in loader:
                ...
                with torch.no_grad():
                    ...
                    running_acc += torch.sum((y_pred.reshape(len(y_pred),1) == a.long()) )
                num_updates += 1
...
def main():     
    exp = SANExperiment(output_dir="./yy/lr1e-3_bs24_resnet50", batch_size=24, train_set=train_set, lr=1e-3,num_epochs=30)
    exp.run()
